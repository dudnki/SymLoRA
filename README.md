...
본 연구는 PiSSA-style SVD로 A를 초기화한 뒤 **동결**하는 단순한 결합이 정답임을 실증합니다. 더 나아가 LoRA를 `A=구조 anchor` × `B=task freedom`의 분업 관점에서 해석하고, 학습 후 가중치가 초기 부분공간을 얼마나 벗어났는지를 측정하는 지표 **W_rel** 을 도입해 그 메커니즘을 정량화합니다.
